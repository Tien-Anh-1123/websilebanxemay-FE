...
const ModalEditProduct = (props) => {
  ...
  const [brandId, setBrandId] = useState(productEdit.BrandId);
  const [categoryId, setCategoryId] = useState(productEdit.CategoryId);
  const [description, setDescription] = useState("");
  const { listCategory, listBrand } = useSelector(
            ...
            <div className="row">
              ...
              <div className="col-3">
                <div className="mb-3">
                  ...
                  >
                    <option value={brandId}>Chọn hãng xe</option>
                    {listBrand &&
                      listBrand.length > 0 &&
                      listBrand.map((item, index) => {
                        return (
                          <option key={index + 1 + "brand"} value={item.id}>
                            {item.name}
                          </option>
                        );
                      })}
